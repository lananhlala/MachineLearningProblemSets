...
class tiki(CrawlSpider):
    ...
    def __init__(self, url='https://tiki.vn/sach-ba-me-em-be/c2527', start_page='1', end_page='21', *args, **kwargs):
    ...
    def parse_first(self, response):
        ...
        for item in data_ids:
            yield SplashRequest(url='https://tiki.vn/api/v2/reviews?product_id={}&limit=200'.format(item), callback=self.parse_item, endpoint='execute', args={'lua_source' : self.get_link_scripts})

    def parse_item(self, response):
        data_get = json.loads(response.css('pre::text').extract_first())
        for item in data_get['data']:
            _data = {}
            _data['product_id'] = item['product_id']
